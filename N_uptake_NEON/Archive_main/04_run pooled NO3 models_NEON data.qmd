...
print(fit.bigc19,  pars=c("b0", "b1", "sigma_U", "sigma"), digits=2)
print(fit.bigc19, pars=c("K"), digits=2)
...
conc_pred <- rstan::extract(fit.bigc19, pars = "conc_pred")$conc_pred 
...
avg_conc_pred_MA <- apply(conc_pred, MARGIN = c(2, 3), FUN = mean)  

 
#avg_conc_pred.oe <- as.vector(c(avg_conc_pred_oeMA))
N_conc_pred <- as.vector(c(avg_conc_pred_MA))
...
N_output.df <- data.frame(local_datetime, hours, mod_hours, model_datetime, model_day, N_conc, N_conc_pred)
...
N_and_Nhat <- N_output.df %>%
  ...
  ggtitle("NEON: Big Creek 2019 pooled model") +
...
Nhat_V_N <- ggplot(data = N_output.df, aes(x=N_conc, y=N_conc_pred)) +
  geom_point() + 
...
N_amb <- mean(cari.df.19h$surfWaterNitrateMean) # 27.4, 4.24 mg/m^3  sd = 3.1
N_sd <- sd(cari.df.19h$surfWaterNitrateMean)
N_e <- rnorm(length(cari.df.19h$surfWaterNitrateMean), mean=N_amb, sd=N_sd)
...
saveRDS(fit.cari19, here("N_uptake_NEON/data/model_fits/cari19.fit.rds"))
...
# Get conc_pred from fit; plot vs concMA
conc_pred <- rstan::extract(fit.cari19, pars = "conc_pred")$conc_pred 
#conc_pred.oe <- extract(fit.oe, pars = "conc_pred")$conc_pred 
...
U_vs_time <- ggplot(data = U_output.df, aes(x=mod_day, y=U_mod_avg)) +
  geom_point() + 
  # geom_point(y = sumlight.real, color = 'gold') +
  # ADD IN HIGH AND LOW CIs
...
cari_ts <- ggplot(data=N_output.df, aes(x=model_day, y=N_conc)) + 
  geom_point() +
  xlab("Datetime") + ylab("N (umol/L)") + 
  ggtitle("Caribou Creek N timeseries") + 
  theme_bw()
...
U_vs_light <- ggplot(data = U_output.df, aes(x=sumlight.real, y=U_mod_avg)) +
  ...
  #xlab("true light (satellite)") + ylab("modeled U (mmol/m2/day)") +
  xlab("light (satellite)") + ylab("modeled U (mmol/m2/day)") +
  ggtitle("Caribou Creek 2019: scatterplot of NO3 uptake and daily light") +
  #ylim = c(-0.2, 1) +
  theme_bw()
...
K_time <- ggplot(data = K_output.df, aes(x=mod_day)) +
  ...
  xlab("Julian day") + ylab("modeled K (day -1)") + # daily change in N concentration
  ylim(0,20) +
  ggtitle("modeled K over time, Caribou Creek 2019 pooled model w real light") +
  theme_bw()
...
####### U vs K _______________________

UvsK <- ggplot(data=comp.df, aes(x=p.U_mod_avg, y=p.K_mod_avg)) +
  geom_point() +
  xlab("process model U (mmol m-2 d-1)") + ylab("process model K (d-1)") + 
  ggtitle("K vs U for the process model (Caribou Creek 2019 data)") +
  theme_bw()
...
# Get conc_pred from fit; plot vs concMA
conc_pred <- extract(fit.king19, pars = "conc_pred")$conc_pred 
#conc_pred.oe <- extract(fit.oe, pars = "conc_pred")$conc_pred 
...
N_output.df <- data.frame(local_datetime, hours, model_day, N_conc, N_conc_pred)
...
quartz()
ggplot(data = N_output.df, aes(x=N_conc, y=N_conc_pred)) +
  geom_point() + 
