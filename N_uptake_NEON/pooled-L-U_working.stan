...
parameters {
    ...
    vector [D] logK;            // Nitrate turnover rate (day^-1)
...
model {
    ...
    logK[d] ~ normal(logK_mean, sigma_logK);                  // simple pooling w. mean K + sd; mb move to priors section, below?
    ...
    // so nope, nvmd, can't separate those out. Use conc_pred in the generated quantities block instead.
            }
         }
    ...
    sigma_U~normal(0, 0.2);//half normal distribution - adjusted, was normal(0, 4.57)
     ...
     logK_mean~normal(2,1.6);  //informed, yet broad prior from LINX2
...
generated quantities {
      ...
      matrix[T,D] conc_pred;      // does not carry through from model block. Therefore we are adding it here (can't be in transformed params to carry through, needs to be here - ROH).
      vector [D]  U_tot;
      vector [D] Vf_tot;
      ...
      U_tot[d] = K[d]*N_e[d]*zMA[1,d];   // actually 'U_other'... right?
      Vf_tot[d] = K[d]*zMA[1,d];
