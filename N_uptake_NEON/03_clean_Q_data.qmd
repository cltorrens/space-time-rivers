...
# bigc.q.21.daily <- bigc.q.21 %>%
#   dplyr::group_by(model_jday) %>%
#   
...
remove.2123 <- bigc.q.2123 %>% 
  count(yr_jday) %>%
  filter(n != 1440) 
  
remove.2123.2 <- bigc.q.2123 %>% 
...
list.23 <- c(132, 133, 134, 139, 140, 141, 158, 159, 160, 161, 162, 163, 164, 165, 166, 170, 171, 172, 173, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 191, 192, 193, 194, 196, 197, 198, 199, 200, 201, 202, 203, 204, 205, 208, 209, 210, 211, 212, 213,  214, 215, 216, 217, 218, 219, 220, 221, 223, 224, 252, 253, 254, 256, 257, 258, 259, 263, 264, 265, 266, 267, 268, 269, 270, 271, 274, 275, 276, 277, 279, 280, 281, 282, 283, 284, 287, 288, 290, 291) # rm 285 - flagged, big spike
...
NAdays <- cari.q.2123 %>%
  filter(is.na(maxpostDischarge)) %>%   # Filter rows where 'no3mean' is NA
  select(yr_jday)  
unique(NAdays)

# 1 2021_144 (9)
# 2 2021_270 (59 continuous: 1h)
# 3 2021_271 (LOTS - about 1440 (so, all day))
# 4 2021_272 (all day)
# 5 2021_273 (720, so half the day)

which(NAdays$yr_jday == "2021_144")

flagdays <- cari.q.2123 %>%
  filter(cari.q.2123$dischargeFinalQF == 1) %>%   # Filter rows where 'no3mean' is NA
  select(yr_jday)  
unique(flagdays)
# 1 2021_144
# 2 2021_270
...
# Remove days 2021_271:271, 2023_285

# 2023-10-03, model_jday ??? : removing spike, filling gap

cari.q.2123 <- cari.q.2123 %>%
  mutate(
    maxpostDischarge = if_else(date(model_datetime) == as.Date("2023-10-03") & maxpostDischarge > 490, NA_real_, maxpostDischarge)
  )
...
# cupe.q.2123 <- prep2123(bind_rows(cupe.q.21, cupe.q.22, cupe.q.23))  #works!

remove.2123 <- cupe.q.2123 %>% 
  count(yr_jday) %>%
  filter(n != 1440)
...
which(NAdays$yr_jday == "2021_246")

flagdays <- cupe.q.2123 %>%
  filter(cupe.q.2123$dischargeFinalQF == 1) %>%   # Filter rows where 'no3mean' is NA
  select(yr_jday)  
unique(flagdays)
# same ones - probably made NA by the 2021-NA code above
#  1 2021_19 
#  2 2021_26 
#  3 2021_27 
#  4 2021_40 
...
# Fill the gap I created: 
maxgap <- 30  # set the maximum gap for zoo() to fill
...
# cupe.q.2123.daily <- dailysum(cupe.q.2123) works!

zeroQ <- cupe.q.2123.daily %>%
  dplyr::filter(Qmean_ls == 0)
# none
...
prin.q.2123 <- prep2123(bind_rows(prin.q.21, prin.q.22, prin.q.23))
...
prin.q.2123.daily <- dailysum(prin.q.2123)

zeroQ <- prin.q.2123.daily %>%
  dplyr::filter(Qmean_ls == 0)
# none
...
wlou.q.2122 <- prep2123(bind_rows(wlou.q.21, wlou.q.22))

remove.2122 <- wlou.q.2122 %>% 
  count(yr_jday) %>%
  filter(n != 1440)
...
Q.NA <- which(is.na(wlou.q.2122$maxpostDischarge))
Q.NA

Q.flag <- which(wlou.q.2122$dischargeFinalQF == 1)
...
flagdays <- wlou.q.2122 %>%
  filter(wlou.q.2122$dischargeFinalQF == 1) %>%   # Filter rows where 'no3mean' is NA
  select(yr_jday)  
unique(flagdays)
# same ones - probably made NA by the 2021-NA code above
...
wlou.q.2122.daily <- dailysum(wlou.q.2122)

zeroQ <- wlou.q.2122.daily %>%
  dplyr::filter(Qmean_ls == 0)
# 40 days!
...
wlou.q.2122.daily <- Q_final

zeroQ <- wlou.q.2122.daily %>%
  dplyr::filter(Q_cms == 0 | is.na(Q_cms))
# Just 13 left! 

wlou.q.2122.daily <- wlou.q.2122.daily %>%
