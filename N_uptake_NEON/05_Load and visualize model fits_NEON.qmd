...
gpp.df <- read_csv(file=here("N_uptake_NEON/data/GPP_Marzolf_publ/8_NEON_metab_estimates.csv")) %>%
  mutate(date = mdy(date)) %>%
  filter(year(date) %in% 2021:2023)
...
linx.df <- read_csv(here("N_uptake_NEON/data/LINX_II_raw_data.csv")) %>%
  select(state, stream, Date_15Nexpt, water_velocity_m_min,Q_L_s,NO3conc_ugN_L, GPP_gO2_m2_d, width_m, depth_m, Vf_tot_no3_cm_s, Vfdentot_cm_s, Vf_assim_cm_s) %>%
...
daily_output_bigc.df <- bigc.meanqi %>%
  ...
  select(site, yr_jday, K, K.lower, K.upper, K.median, U, U.lower, U.upper, U.median, N_e, N_e.lower, N_e.upper, N_e.median, sumlightReal, sumlightIdeal, Q, v, z, b0,b1, model_date, model_jday, Year, domainID, d) #d is the index from the meanqi object - used for plotting 'ts' by index
...
One day per site has super-high K relative to the other Ks
(later, should likely re-run model w/o these days)
```{r remove outlier}
...
colorblind_palettes
# Archambault works - but I couldn't get it to visualize. Moving on to PNW palettes...
...
UvKplot <- ggplot(data=daily_summary_all.df, aes(x=K, y=U, color=site)) +
  ...
  scale_x_log10() + scale_y_log10() + 
  # facet_wrap(~site) +
  labs(y=expression("K"~(d^-1)),
       x=expression("Autotrophic uptake (U)"~(mmol~m^-2~d^-1))
        ) + 
...
UvGPP <- ggplot(data=U_GPP, aes(x=GPP.mmol, y=U, color=site)) + 
  ...
  geom_abline(intercept= 0, slope = 1/40, linetype = "twodash", color='grey') +  #1:20
  geom_abline(intercept= 0, slope = 1/60, linetype = "twodash", color='grey') +  #1:30
  # geom_abline(intercept= 0, slope = 1/70, linetype = "dotted", color='grey') +
  geom_abline(intercept= 0, slope = 1/80, linetype = "twodash", color='grey') + #1:40
  # geom_abline(intercept= 0, slope = 1/90, linetype = "dotted", color='grey') +
  geom_abline(intercept= 0, slope = 1/100, linetype = "twodash", color='grey') +# 1:50
  # geom_abline(intercept= 0, slope = 1/110, linetype = "dotted", color='grey') +
  geom_abline(intercept= 0, slope = 1/120, linetype = "twodash", color='grey') + #1:60
   geom_abline(intercept= 0, slope = 1/140, linetype = "twodash", color='grey') + #1:70
  labs(x=expression("GPP"~(mmol~O[2]~m^-2~d^-1)),
...
pal_extended <- c(pal, "LINX II" = "darkgrey")
...
# create the plot
Vf_plot <- ggplot() + 
  geom_point(data=daily_summary_all.df, aes(x=N_e, y=Vf_auto, color=site_legend)) + 
  geom_point(data=linx.df, aes(x=N_conc, y=Vf_tot, color=site_legend)) + 
