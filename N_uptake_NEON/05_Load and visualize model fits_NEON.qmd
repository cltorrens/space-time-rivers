...
plot_UvNe <- function(data, plot_title) {
  ...
  geom_errorbar(aes(ymin = U.lower, ymax = U.upper), width = 0.2, color='grey70') + # CI bars
  geom_point() + 
...
plot_K_ts <- function(data, plot_title) {
  ...
  geom_errorbar(aes(ymin = K.lower, ymax = K.upper), width = 0.2, color='grey70') + # CI bars
  geom_point() + 
...
N_output_bigc.df <- bigc.df.h %>%  
  ...
  rename(N_conc = surfWaterNitrateMean, 
         lightReal_wm2 = GHI_wm2) %>%
   mutate(hours = hour(local_datetime), 
         mod_hours = hour(model_datetime)) %>%
  select(site, local_datetime, model_datetime, N_conc, N_pred, N_pred.lower, N_pred.upper, lightReal_wm2, z, Year, yr_jday, Jday, model_jday, hours, mod_hours) 
...
daily_output_bigc.df <- bigc.meanqi %>%
  add_column(yr_jday = unique(bigc.df$yr_jday), 
             sumlightReal = bigc.modeldata$sumlightReal, 
             sumlightIdeal = bigc.modeldata$sumlightIdeal, 
             z = colMeans(bigc.modeldata$zMA), # daily depth = average of hourly depth
             domainID = domainID) %>%
  ...
  select(site, yr_jday, K, K.lower, K.upper, K.median, U, U.lower, U.upper, U.median, N_e, N_e.lower, N_e.upper, N_e.median, sumlightReal, sumlightIdeal, z, model_date, model_jday, Year, domainID, d) #d is the index from the meanqi object - used for plotting 'ts' by index
...
daily_output_cari.df <- cari.meanqi %>%
  add_column(yr_jday = unique(cari.df$yr_jday), 
...
daily_output_cupe.df <- cupe.meanqi %>%
  add_column(yr_jday = unique(cupe.df$yr_jday), 
...
daily_output_wlou.df <- wlou.meanqi %>%
  add_column(yr_jday = unique(wlou.df$yr_jday), 
             sumlightReal = wlou.modeldata$sumlightReal, 
             sumlightIdeal = wlou.modeldata$sumlightIdeal, 
             z = colMeans(wlou.modeldata$zMA), # daily depth = average of hourly depth
             domainID = domainID) %>%
  separate(yr_jday, into = c("Year", "model_jday"), sep = "_", convert = TRUE, remove = FALSE) %>%  # makes the new columns numeric if possible, and retains the old column
  mutate(model_date = ymd(paste0(Year, "-01-01")) + days(model_jday - 1)) %>%
  select(site, yr_jday, K, K.lower, K.upper, K.median, U, U.lower, U.upper, U.median, N_e, N_e.lower, N_e.upper, N_e.median, sumlightReal, sumlightIdeal, z, model_date, model_jday, Year, domainID, d) #d is the index from the meanqi object - used for plotting 'ts' by index
