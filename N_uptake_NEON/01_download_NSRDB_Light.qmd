...
## The data downloads by 1 site and 1 year - this for loop downloads 2021-23 with one command
...
bind_nsrdb  <- function(path, site) {
  files  <- list.files(path=path, pattern=site) %>%
    lapply(read.csv, skip = 2) %>%
    dplyr::bind_rows() %>%
    dplyr::select(1:7) %>%
    dplyr::rename("year" = 1, "month"= 2, "day"= 3, "hour"= 4, "minute" = 5, "clearsky_GHI_wm2"= 6, "GHI_wm2"= 7) %>% 
    dplyr::mutate(local_datetime = make_datetime(year, month, day, hour, minute)) %>%
    dplyr::select(local_datetime, GHI_wm2, clearsky_GHI_wm2)
}
