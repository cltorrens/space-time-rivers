...
csv.name.bigc19 <- 'BIGC_37.05767_-119.25538_2019_30.csv'
csv.name.bigc21 <- 'BIGC_37.05767_-119.25538_2021_30.csv'
csv.name.cari19 <- 'CARI_65.15307_-147.50195_2019_30.csv'
csv.name.cari20 <- 'CARI_65.15307_-147.50195_2020_30.csv'
csv.name.king19 <- 'KING_39.1046_-96.60264_2019_30.csv'
csv.name.king20 <- 'KING_39.1046_-96.60264_2020_30.csv'
csv.name.walk19 <- 'WALK_35.95722_-84.27921_2019_30.csv'
csv.name.walk22 <- 'WALK_35.95722_-84.27921_2022_30.csv'

## Change wd to get files
setwd(here('N_uptake_NEON/data/NSRDB_light_data'))

bigc19_real.sumlight <- read_csv(csv.name.bigc19, skip=2) %>%
  mutate(Datetime = ymd_hm(paste(Year, Month, Day, Hour, Minute)),  #create a datetime field
         Time = format(Datetime, format = "%H:%M"),
         Jday = yday(Datetime))

bigc21_real.sumlight <- read_csv(csv.name.bigc21, skip=2) %>%
  mutate(Datetime = ymd_hm(paste(Year, Month, Day, Hour, Minute)),  #create a datetime field 
         Time = format(Datetime, format = "%H:%M"),
         Jday = yday(Datetime))

cari19_real.sumlight <- read_csv(csv.name.cari19, skip=2) %>%
  mutate(Datetime = ymd_hm(paste(Year, Month, Day, Hour, Minute)),  #create a datetime field 
         Time = format(Datetime, format = "%H:%M"),
         Jday = yday(Datetime))

cari20_real.sumlight <- read_csv(csv.name.cari20, skip=2) %>%
  mutate(Datetime = ymd_hm(paste(Year, Month, Day, Hour, Minute)),  #create a datetime field 
         Time = format(Datetime, format = "%H:%M"),
         Jday = yday(Datetime))

king19_real.sumlight <- read_csv(csv.name.king19, skip=2) %>%
  mutate(Datetime = ymd_hm(paste(Year, Month, Day, Hour, Minute)),  #create a datetime field 
         Time = format(Datetime, format = "%H:%M"),
         Jday = yday(Datetime))

king20_real.sumlight <- read_csv(csv.name.king20, skip=2) %>%
  mutate(Datetime = ymd_hm(paste(Year, Month, Day, Hour, Minute)),  #create a datetime field 
         Time = format(Datetime, format = "%H:%M"),
         Jday = yday(Datetime))

walk19_real.sumlight <- read_csv(csv.name.walk19, skip=2) %>%
  mutate(Datetime = ymd_hm(paste(Year, Month, Day, Hour, Minute)),  #create a datetime field 
         Time = format(Datetime, format = "%H:%M"),
         Jday = yday(Datetime))

walk20_real.sumlight <- read_csv(csv.name.walk20, skip=2) %>%
  mutate(Datetime = ymd_hm(paste(Year, Month, Day, Hour, Minute)),  #create a datetime field 
         Time = format(Datetime, format = "%H:%M"),
         Jday = yday(Datetime))
