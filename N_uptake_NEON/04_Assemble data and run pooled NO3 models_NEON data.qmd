...
z <- 0.2 #~mean depth in m based on Kelly Aho's dataset (rounded up from ~ 1.5) - constant for now
...
cari.df.h <- read_csv(path_h) %>%
  mutate(local_datetime = with_tz(local_datetime, tzone="US/Alaska"), 
         model_datetime.x = local_datetime - hours(2)) %>%
  rename(model_datetime = model_datetime.x) %>%
  select(-model_datetime.y)
...
z <- 0.33 #mean depth in m based on Kelly Aho's dataset - constant for now
zMA<- matrix(z, ncol = nday, nrow=24) #depth at each timestep as matrix (col=days, row= hours/day)
...
z <- 0.15 #mean depth in m based on Kelly Aho's dataset - constant for now
zMA<- matrix(z, ncol = nday, nrow=24) #depth at each timestep as matrix (col=days, row= hours/day)
...
## Standardize terms for model
light <- prin.df.h$mod_light    #bigc.df.19h$light
...
z <- 0.2 #~ mean depth in m based on Kelly Aho's dataset (generously rounded up from 1.5) - constant for now
zMA<- matrix(z, ncol = nday, nrow=24) #depth at each timestep as matrix (col=days, row= hours/day)
...
## Standardize terms for model
light <- syca.df.h$mod_light    #bigc.df.19h$light
...
z <- 0.2 #~ mean depth in m based on Kelly Aho's dataset (generously rounded up from 1.4) - constant for now
zMA<- matrix(z, ncol = nday, nrow=24) #depth at each timestep as matrix (col=days, row= hours/day)
