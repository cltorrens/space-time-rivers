...
mmhg <- bpcalc(29.9, 3350/3.28)# elevation at Nyack
...
all_oxy$osat<- osat(all_oxy$Temperature_degC, 671)
...
all_oxy_aug23 <- all_oxy %>%
  ...
  filter(jday > 234 & jday < 242)

all_oxy_oct23 <- all_oxy %>%
  mutate(jday = yday(MST_datetime)) %>%
  filter(jday > 273 & jday < 283)
...
augBC_data_sm<-data.frame(DO.obs=all_oxy_aug23$DO_mgl, DO.sat=all_oxy_aug23$osat, 
                          temp.water=all_oxy_aug23$Temperature_degC, depth=rep(0.50,length(all_oxy_aug23$Temperature_degC)), 
...
save(augBC_fit, file="augBC_fit.RData")

plot_DO_preds(predict_DO(augBC_fit))
plot_metab_preds(predict_metab(augBC_fit))
get_params(augBC_fit , uncertainty='ci')

augGPPavg <- (2.703411+2.771905+2.667298+2.815274+2.864636)/5

### October week - Beaver Cr

octBC_data_sm<-data.frame(DO.obs=all_oxy_oct23$DO_mgl, DO.sat=all_oxy_oct23$osat, 
                          temp.water=all_oxy_oct23$Temperature_degC, depth=rep(0.50,length(all_oxy_oct23$Temperature_degC)), 
